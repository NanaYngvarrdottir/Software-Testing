...
string ThisGridName = "YOUR GRID NAME GOES HERE";
string OtherGridName = "Second Life";
string MyConversionSymbol = "G2L"; 
string OtherConversionSymbol = "L2G";
string myMoneySymbol = "G$";
string otherMoneySymbol = "L$";
...
{    
    ...
    listen( integer channel, string name, key id, string msg )
    {
        ...
        if (msg == "CORRECT")
        {
            ...
            message += "Payout Code|" + MyConversionSymbol + "|" +  transactionID + "|" + llList2String(nameOfUser, spot) + "|" + (string)amount + "|" + (string)llFloor((float)amount / 2.0);
        ...
        else
        {
            ...
            if (llList2String(results, 0) == "Payout Code")
            {
                ...
                else if (llList2String(results, 1) == OtherConversionSymbol)
                {
                    ...
                    {
                        key avatarKey = aaAvatarFullName2Key(llList2String(results, 3));
                        if (avatarKey == NULL_KEY)
                        {
                            llOwnerSay("User " + llList2String(results, 3) + " can not be found. Please manually pay out try to figure out who she/he is and pay them.");
                        }
                        else
                        {
                            llGiveMoney(avatarKey, llList2Integer(results, 5));
                            hasRanTransaction += [llList2String(results, 2)];
